...
function Emojis(){
  ...
  //console.log("setEndpoint",endpoint);
    return(products? <div className="flex">
        <nav className="w-[18%] shadow-zinc-500/50 shadow-lg h-screen bg-pink-50 py-4 items-center flex flex-col">
            <h1 className="w-[80%] text-center  py-2 my-4 text-2xl border border-zinc-950 ">Emojika {"; )"}</h1>
            <div className="w-[90%]">
            ...
            <h1 className="px-4 py-2 text-2xl">♥︎ Favourite</h1>
            <Link to={"/About"} className="px-4 py-2 text-2xl">? About</Link>
            </div>
        </nav>
        <div className="h-scrren w-[82%]">
          <nav className="h-[15vh] w-full p-6 flex justify-between items-center ">
            <h1 className="text-semibold text-xl ml-10" >Devices</h1>
            <div className="flex w-[50%]">
            <input
                className="w-[70%] text-xl px-4 bg-zinc-100 mr-10 border-zinc-200 border py-2 rounded h-[8vh]"
                type="text"
                placeholder="Search"
          ...
          <div className="h-[85vh] items-center justify-center p-5 w-full overflow-y-auto flex flex-wrap">
          {filteredResults.map((p,i)=>(
               <Link to={`/emojis/${p.slug}/${p.character}/${p.group}/${p.subGroup}`} key={i}  className="hover:scale-110 hover:h-[16vh] bg-red-100 h-[14vh] flex flex-col shadow-lg justify-center items-center w-[8%] rounded-md m-4 group hover:shadow-2xl transition-shadow duration-300"> 
